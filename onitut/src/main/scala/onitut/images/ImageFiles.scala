...
object ImageFiles:
  ...
  def hashOf(bytes: Array[Byte]): String =
    hasher.digest(bytes) map ("%02X".format(_)) mkString
   ...
   */
  private def contents(folder: File): List[FileOrLink] =
    folder.listFiles.toList flatMap scan
  ...
  def makeBak(path: Path): Path =
    val bak = Paths.get(path.toString + ".bak")
    try
   ...
   * @param photoDir the folder where all photos are supposed to be stored
   ...
   * @param photoDir photo folder path
   */
